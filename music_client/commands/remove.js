...
module.exports = {
     ...
     */
     ...
     if(me.voice?.channelId && channel.channelId !== me.voice.channelId) return interaction.reply({embeds:[new Discord.EmbedBuilder()
      ...
      try {
       const remove = await queue.songs.splice(args._hoistedOptions[0].value-1,1);
        return interaction.reply({embeds:[new Discord.EmbedBuilder()
          .setColor("Green")
          .setAuthor({name:`Song ${args._hoistedOptions[0].value} removed from the queue. ✅`,iconURL:client.user.displayAvatarURL()})
        ]});
      } catch (error) {
        return interaction.reply({embeds:[new Discord.EmbedBuilder()
          .setColor('Red')
          .setAuthor({name:`There's no songs in that position.`,iconURL:displayAvatarURL()})
        ]});
      }
